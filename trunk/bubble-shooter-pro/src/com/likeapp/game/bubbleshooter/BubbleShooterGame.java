...
public class BubbleShooterGame extends GameScreen {
  ...
  public BubbleShooterGame(BmpWrap background_arg,
                    BmpWrap[] bubbles_arg,
  ...
  private void frozenify()
  {
    ...
    if (frozenifyX < 0) {
      ...
      if (frozenifyY<0) {
        ...
        soundManager.playSound(BubbleShooterActivity.SOUND_NOH);
  ...
  {
    soundManager.playSound(BubbleShooterActivity.SOUND_NEWROOT);
      ...
      for (int j=0 ; j<12 ; j++) {
        if (bubblePlay[i][j] != null) {
          ...
          if (bubblePlay[i][j].getSpritePosition().y>=380) {
            ...
            soundManager.playSound(BubbleShooterActivity.SOUND_LOST);
          }
        }
      }
    }
                      ...
                      double trackball_dx, double[] touch_dx)
  {
    ...
    if (BubbleShooterActivity.getDontRushMe()) {
      hurryTime = 1;
    }

    if(key_fire && touch_dx != null){
    	//mTouchDX = Math.acos(-(x-mCanvasWidth/2) * 2 / mCanvasWidth) * 40 / Math.PI;
//        launchBubblePosition = touch_dx;
    	double x_r = -(touch_dx[0] - launchBubble.getXCenter());
    	double y_r =touch_dx[1] - launchBubble.getYCenter();
    	
    	/*Log.i("sloan", "touch x:"+touch_dx[0]);
    	Log.i("sloan", "touch y:"+touch_dx[1]);
    	Log.i("sloan", "xCenter:"+launchBubble.getXCenter());
    	Log.i("sloan", "yCenter:"+launchBubble.getYCenter());
    	Log.i("sloan", "x_r:"+x_r);
    	Log.i("sloan", "y_r:"+y_r);*/
    	
    	launchBubblePosition = Math.acos(x_r/Math.sqrt(x_r * x_r + y_r * y_r)) * 40 / Math.PI;
    ...
    if (endOfGame) {
      ...
      if (move[FIRE] == KEY_UP || hurryTime > 480) {
    	  //shootTimes++;
    	 // Log.i("jackyli","shoot="+shootTimes);
    	//shootTimesint shootTimes = getShootTimes();
         
    		  long shootNumber =GameConfig.getInstance().get(BubbleShooterActivity.SHOOT_NUMBER_OF_SKIP_CHANCE_KEY, 0L)+1;
    		  GameConfig.getInstance().put(BubbleShooterActivity.SHOOT_NUMBER_OF_SKIP_CHANCE_KEY, shootNumber);	 
    		  Log.i("jackyli","shootTimes:"+(shootNumber));
    		 // shootTimes =0;
        ...
        if (movingBubble == null && readyToFire) {
          ...
          soundManager.playSound(BubbleShooterActivity.SOUND_LAUNCH);
    ...
    if (movingBubble != null) {
      movingBubble.move();
      //Log.i("jackylil","fixed="+movingBubble.fixed());
      if (movingBubble.fixed()) {
        if (movingBubble.getSpritePosition().y>=380 &&
          ...
          soundManager.playSound(BubbleShooterActivity.SOUND_WON);
        } else {
          fixedBubbles++;
          blinkDelay = 0;
          
          int MAX_FIXED_BUBBLES = levelManager.getMaxFixedBubbleCount();
          //Log.i("jackylil","fixedBubbles="+fixedBubbles+",MAX_FIXED_BUBBLES="+MAX_FIXED_BUBBLES);
          if (fixedBubbles >= MAX_FIXED_BUBBLES) {
            fixedBubbles = 0;
            sendBubblesDown();
          }
        }
    ...
    if (movingBubble == null && !endOfGame) {
      ...
      if (hurryTime>=240) {
        if (hurryTime % 40 == 10) {
          ...
          soundManager.playSound(BubbleShooterActivity.SOUND_HURRY);
