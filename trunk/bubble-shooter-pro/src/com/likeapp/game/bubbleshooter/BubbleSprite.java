...
public class BubbleSprite extends Sprite
{
  ...
  public BubbleSprite(Rect area, int color, BmpWrap bubbleFace,
                      ...
                      SoundManager soundManager, BubbleShooterGame frozen)
  {
    super(area);
  ...
  public void move()
  {
    ...
    if (realX>=414.) {
      moveX = -moveX;
      realX += (414. - realX);
      soundManager.playSound(BubbleShooterActivity.SOUND_REBOUND);
    } else if (realX<=190.) {
      moveX = -moveX;
      realX += (190. - realX);
      soundManager.playSound(BubbleShooterActivity.SOUND_REBOUND);
    ...
    if (checkCollision(neighbors) || realY < 44.+frozen.getMoveDown()) {
      ...
      if (checkJump.size() >= 3) {
        ...
        soundManager.playSound(BubbleShooterActivity.SOUND_DESTROY);
      } else {
        bubbleManager.addBubble(bubbleFace);
        ...
        moveY = 0.;
        fixedAnim = 0;
        soundManager.playSound(BubbleShooterActivity.SOUND_STICK);
      }
    }
